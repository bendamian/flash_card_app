...
# add french:
def next_card():
    current_card = random.choice(js)
    canvas.itemconfig(c_title,text='French')
    canvas.itemconfig(c_word,text=current_card['French'])
...
c_title = canvas.create_text(400, 150, text="Title", font=("Ariel", 40, "italic"))
c_word= canvas.create_text(400, 260, text="Title", font=("Ariel", 60, "bold"))
right = tk.Button(window, image=right_b, height=100, width=100, pady=10, padx=10, command=next_card)
wrong = tk.Button(window, image=wrong_b, height=100, pady=10, padx=10,command=next_card)
# define a grid
